...
use Flexper\Exception\UniqidDbengineNotSupportException;

use Flexper\Exception\MissingConfigException;
use Flexper\Exception\CreateUniqidFailedException;
use Flexper\Exception\UniqidTypeUndefinedException;
...
class Uniqid{
    ...
    private static function init(){
        if (!self::$_isInited){
	        $config = Env::getInstance('\Flexper\Config')->uniqid;
	        if (!isset($config)){
	        ...
	        $className = 'Flexper\Uniqid\Dbengine\\' . ucfirst($config['dbengine']);
